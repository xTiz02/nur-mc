    ...
    async executeActions(actions, requestId) {
        ...
        try {
            for (let i = 0; i < actions.length; i++) {
                ...
                console.log(`[${requestId}] Ejecutando acción ${i + 1}/${actions.length}: ${action.name}`);
                ...
                try {
                    ...
                    const result = await Promise.race([
                        ...
                        new Promise((_, reject) =>
                            setTimeout(() => reject(new Error('Timeout')), config.ACTION_TIMEOUT)
                        )
                    ]);
                    ...
                    if (this.shouldStopOnError(action.name, error)) {
                        console.log(`[${requestId}] Deteniendo ejecución por error crítico`);
                        break;
                    }
                }
            }
