...
const primitiveActions = {
  ...
  placeBlock: {
    ...
    execute: async (bot, params) => {
      ...
      if (!referenceBlock) {
        throw new Error(
            "No puedo encontrar un bloque de referencia para colocar");
      }
  ...
  collectNearby: {
    ...
    execute: async (bot, params) => {
      ...
      if (items.length === 0) {
        return `No encontré items${itemType ? ` de tipo ${itemType}`
            : ''} cerca`;
      }

      for (const item of items.slice(0, 5)) {
        const goal = new goals.GoalNear(item.position.x, item.position.y,
            item.position.z, 1);
        await bot.pathfinder.goto(goal);
